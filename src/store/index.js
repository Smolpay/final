...
export default new Vuex.Store({
  ...
  getters: {
    ...
    cartFormInfoProducts(state) {
      return (state.orderInfoProducts = state.orderInfo.basket.items.map(
        (infoProducts) => infoProducts.product
      ));
    },

    cartFormInfoProductsAmount(state) {
      return (state.orderInfoProducts = state.orderInfo.basket.items
        .map((infoProducts) => infoProducts)
    ...
    cartFormInfoTotalPrice(state) {
      return state.orderInfo.basket.items
        .map((infoProducts) => infoProducts)
        .reduce(
          (infoTotal, item) => item.product.price * item.quantity + infoTotal,
          0
        );
    },
